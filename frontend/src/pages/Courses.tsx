...
import { FaCreditCard, FaSignInAlt, FaCheckCircle, FaInfoCircle, FaSearch, FaShoppingCart ,FaCommentAlt} from 'react-icons/fa';
import { useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { addCourseToCart, fetchCart } from '../redux/cartSlice';
import API from '../api/client';
import CourseReviewsList from '../User/components/CourseReviewList';
interface Course {
    id: number;
    title: string;
    description: string;
    ...
    <div className="d-flex align-items-center justify-content-between mb-3 bg-light p-2 rounded-3">
        <div className="d-flex align-items-center gap-2">
            {course.average_rating && course.average_rating > 0 ? (
                <>
                    <span className="fw-bold text-dark mb-0 small">{course.average_rating}</span>
                    <span style={{ color: '#ffc107', fontSize: '1rem', marginTop: '-2px' }}>
                        {"★".repeat(Math.round(course.average_rating))}
                        {"☆".repeat(5 - Math.round(course.average_rating))}
                    </span>
                </>
            ) : (
                <span style={{ color: '#cbd5e1', fontSize: '1rem', marginTop: '-2px' }}>★★★★★</span>
            )}
        </div>
 
        {/* CLICKABLE TRIGGER TO OPEN REVIEWS LIST POPUP OVERLAY */}
        <Button
            variant="link"
            size="sm"
            className="p-0 text-decoration-none text-primary fw-semibold small d-flex align-items-center gap-1"
            onClick={() => {
    ...
    {/* ========================================================================= */}
            ...
            >
                <Modal.Header closeButton className="border-bottom-0 pt-4 px-4 pb-2">
                    <Modal.Title className="fw-bold h5 text-dark">Student Reviews</Modal.Title>
                </Modal.Header>
                <Modal.Body className="px-4 pb-4 pt-0" style={{ maxHeight: '65vh', overflowY: 'auto' }}>
                    {selectedCourseForModal && (
                        <CourseReviewsList courseId={selectedCourseForModal} />
                    )}
                </Modal.Body>
            </Modal>
            {/* ========================================================================= */}
