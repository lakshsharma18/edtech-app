...
//     // Clean up connections if student unmounts/navigates away from page
//     useEffect(() => {
//         return () => {
//             if (studentWsRef.current) studentWsRef.current.close();
//             if (studentPcRef.current) studentPcRef.current.close();
...
{/* Place this directly inside your WorkSpace.tsx primary Left Column content stack */}
<Col lg={8}>
    {activeLesson && (
        <>
            {/* ... Existing Video Player Cards ... */}
            {/* ... Existing PDF Documentation Material Cards ... */}
 
            {/* ✅ ADD THE UNIFIED REVIEW FORM SECTION HERE */}
            {isEnrolled && (
                <div className="mt-4">
                    <CourseReviewForm
                        courseId={Number(id)}
                        onReviewSubmitted={() => {
                            console.log("🔄 Feedback pipeline successfully refreshed inside user profile engine.");
                        }}
                    />
                </div>
            )}
